...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const response = await ai.models.generateContent({ model: 'gemini-2.5-flash', contents: `Today is ${today}. Find 8 real future cultural moments for this planning brief. Query: ${query}. Brand: ${brand?.name}. Objective: ${brand?.campaignGoal}. Audience: ${brand?.targetAudience}. Region: ${brand?.targetRegion}. Timing: ${brand?.fuzzyTimeframe}. Creativity 0-100: ${brand?.creativityLevel}. Use web search, verify dates and official URLs, exclude past events. Return only a JSON array using fields id,title,date,location,category,description,longDescription,scale,estimatedReach,groundingUrls,websiteUrl,matchScore,matchReason,verificationDate.`, config: { tools: [{ googleSearch: {} }] } });
